...
class LangChainRateLimiterCallback(BaseCallbackHandler):
    ...
    def _adjust_tokens(self, reserved_tokens: int, actual_tokens: int) -> None:
        with self._lock:
            ...
            if self.verbose:
                self.log.debug(
                    f"[LangChainRateLimiterCallback] Token adjustment: Reserved {reserved_tokens}, "
    ...
    def on_llm_start(
        ...
        while not consumed:
            if self.verbose and not has_logged_waiting:
                self.log.warning(
                    f"[LangChainRateLimiterCallback] Target limits reached. Request is waiting because {reason}."
                )
        ...
        if self.verbose and has_logged_waiting:
            self.log.info("[LangChainRateLimiterCallback] Capacity acquired! Resuming request.")
    ...
    async def on_llm_start_async(
        ...
        while not consumed:
            if self.verbose and not has_logged_waiting:
                self.log.warning(
                    f"[LangChainRateLimiterCallback] Target limits reached. Request (async) is waiting because {reason}."
                )
                has_logged_waiting = True
        ...
        if self.verbose and has_logged_waiting:
            self.log.info("[LangChainRateLimiterCallback] Capacity acquired! Resuming async request.")
